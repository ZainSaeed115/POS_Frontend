...
fetchInventoryStats: async () => {
  ...
  try {
    const res = await axiosInstance.get("/product/statistics");
    if (res.data.success) {
      set({ inventoryStats: res.data.stats }); // ✅ store the stats object
    } else {
      set({ inventoryStats: { totalProducts: 0, totalStock: 0, totalValue: 0 } });
    }
  } catch (err) {
